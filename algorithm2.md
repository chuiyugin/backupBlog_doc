...
+ 另外，如果想要寻找最后一个满足“条件 C”的元素的位置，则可以先求第一个满足“条件! C”的元素的位置，然后将该位置减 1 即可（后续小节中**最长回文子串的二分解法**用到了这一点）
...
+ 而由于变成了左开，`left` 的初值要比解的最小值小 1（例如对于下标从 `0` 开始的序列来说，`left` 和 `right` 的取值应为 `-1` 和 `n`），同时语句 `left=mid+1` 应当改为 `left=mid`（**想想**），并且返回的应当是 `right` 而不是 `left`。
+ 代码如下：
```cpp
//解决“寻找有序序列第一个满足某条件的元素的位置”问题的固定模板
//A[]为递增序列，x为欲查询数
...
```
+ 上述模板和前面是等价的，只是在做法上稍有区别而已，可以只使用左闭右闭的模板，但最好能做到流畅推导两种写法。
