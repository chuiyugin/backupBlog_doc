...
+ 块衰落(block fading)与快、慢(slow or fast fading)是不同概念。在现代无线通信中，发送数据通常分割成数据块进行传送。当假定信道在该数据块内部不发生变化时，即：若信道传输特性在固定个字符周期上保持不变，但保持时间比总发送时间要小得多，则可称信道是块衰落的。显然block fading可以认为是一种特殊的slow fading。
