...
## 图论
### 腐烂的橘子
+ 腐烂的橘子：[994.腐烂的橘子](https://leetcode.cn/problems/rotting-oranges/description/?envType=study-plan-v2&envId=top-100-liked)
+ 思路：
	+ 1、遍历整个网格：
		+  统计新鲜橘子数量 `fresh`
		-  把所有腐烂橘子加入队列（作为 BFS 起点）
	- 2、多源 BFS 扩散：
		- `q`（队列中的橘子）：当前所有“正在传播的腐烂橘子”
		- 如果邻居是新鲜橘子：则会被感染，并加入下一轮传播队列
		- 每一层 BFS 结束，表示过了 **1分钟**
	- 3、结束条件
		- 情况一：所有橘子腐烂，返回 `time`
		- 情况二：还有新鲜橘子，返回 `-1`
...
public:
    ...
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        // 从队列中进行广度优先搜索(注意只有存在新鲜橘子的时候才进循环)
...
### 课程表
+ 课程表：[207.课程表](https://leetcode.cn/problems/course-schedule/description/?envType=study-plan-v2&envId=top-100-liked)
+ 思路：
	+ 入度为 `0`，说明这门课没有任何前置条件
	- 这些课程可以作为 BFS 的起点，先学习
	- 同时用 `count` 统计已经可以学完的课程数量
	- BFS 拓扑排序过程：
		- 每次取出队首课程 `cur`，说明这门课已经学完了
		- 然后遍历它的后续课程，并将其入度减 1，如果入度变为 0 即加入队列（表示该门课前置条件完成了可以学）
	- 结束条件：
		- 情况一：如果最终 `count == numCourses`，说明所有课程都能按照拓扑顺序学完，返回 `true`。
		- 情况二：否则，说明还有课程始终无法变成入度 `0`，也就是图中存在环，返回 `false`。
+  代码：
...
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        // 定义节点入度(入度为 0 表示该课程可以先修)
        vector<int> indegree(numCourses, 0);
        // 哈希表(key:先修课程，value:后修课程列表）
...
### 实现 Trie（前缀树）
+ 实现 Trie（前缀树）：[208.实现 Trie（前缀树）](https://leetcode.cn/problems/implement-trie-prefix-tree/description/?envType=study-plan-v2&envId=top-100-liked)
+ 思路：
	+ 1、初始化前缀树根节点
		+ 整棵树需要一个根节点 `root`
		- 根节点本身不存具体字符，只作为所有单词的起点
	- 2、插入单词 `insert`：
		- 从根节点开始，依次遍历单词中的每个字符
		- 如果当前字符对应的子节点不存在，就创建一个新节点
		- 然后继续往下一层走
		- 当整个单词遍历结束时，说明这条路径已经表示了一个完整单词，把末尾节点标记为 `true`
	- 3、查找完整单词 `search`：
		- 同样从根节点开始，按字符逐层向下查找
		- 如果某个字符对应的节点不存在，说明这个单词不在前缀树中，直接返回 `false`
		- 如果所有字符都能找到，还要进一步判断最后一个节点是不是某个单词的结尾
	- 4、查找前缀 `startsWith`：
		- 也是逐个字符向下走
		- 只要中途有字符不存在，就返回 `false`
		- 如果整段前缀都能走完，说明前缀存在，直接返回 `true`
		- 注意⚠️：这里不需要判断 `isEnd`，因为题目只要求“是否存在这个前缀”。
+  代码：
...
public:
    ...
    void insert(string word) {
        ...
        for(int i=0; i<word.length(); i++){
            if(cur->children[word[i]] == nullptr){
                cur->children[word[i]] = new TreeNode();
            }
    ...
    bool startsWith(string prefix) {
        ...
        for(int i=0; i<prefix.length(); i++){
            if(cur->children[prefix[i]] == nullptr)
                return false;
